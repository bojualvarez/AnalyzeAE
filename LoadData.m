...
%% @002 Read HF Data
% Read high frequency data into the workspace
hffile = strcat(Parms.Format.filename2,'_HF_Avg.dat'); % Define the name of the HF file
if exist(hffile,'file') % If this file exists
    fid = fopen(hffile,'r','b'); % Define the file id of the file and allow it to be readable and big endian
else % If hffile does not exist
    hf_avg_file = regexprep(hffile,'_P[0-9]{4,4}_','_'); % Replace the non-averaged file name with a blank
    fid = fopen(hf_avg_file,'r','b'); % Define the file id of this non-average file and allow it to be readable and big endian
end
...
dsize = fread(fid,[1,n],'int32'); % Define the  
...
%% @003 Load LF Data
%
fid = fopen(strcat(bScanParm.Format.filename2,'_LF_Avg.dat'));
LF = fread(fid);